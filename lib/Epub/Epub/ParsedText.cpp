...
uint16_t measureFocusWordWidth(const GfxRenderer& renderer, const int fontId, const std::string& word,
  ...
  if (focusBoundary == 0) {
    return measureWordWidth(renderer, fontId, word, style, letterSpacing, appendHyphen);
  }
  ...
  for (size_t i = 0; i < lineWordCount; i++) {
    ...
    outSuffixX.push_back(boundary == 0
                             ...
                             : measureFocusPrefixAdvance(
                                   renderer, lineWordFonts.empty() || lineWordFonts[i] == 0 ? fontId : lineWordFonts[i],
                                   lineWords[i], lineWordStyles[i], boundary, blockStyle.letterSpacing));
  }
