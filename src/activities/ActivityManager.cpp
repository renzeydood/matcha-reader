...
#include "reader/MangaReaderActivity.h"
...
void ActivityManager::loop() {
  ...
  if (currentActivity) {
    if (!recoveryMode && !currentActivity->isHomeActivity() && mappedInput.wasHomeGesture()) {
    ...
    if (!recoveryMode && mappedInput.hasTouch() &&
    ...
    if (!recoveryMode && currentActivity->name != "FrontlightPanel" &&
...
void ActivityManager::goToReader(std::string path, const bool allowFastInitialRefresh) {
  ...
  if (manga::MangaBook::isMangaFolder(path)) {
    auto activity = makeUniqueNoThrow<MangaReaderActivity>(renderer, mappedInput, std::move(path));
    if (activity)
      replaceActivity(std::move(activity));
    else
      LOG_ERR("ACT", "OOM: manga reader activity");
    return;
  }
