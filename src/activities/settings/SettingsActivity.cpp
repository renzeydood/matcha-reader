...
void SettingsActivity::onEnter() {
  ...
  if (finishOnBack) activeNav().selected = 1;  // category row is locked in embedded mode
...
void SettingsActivity::activateIndex(const int index) {
  ...
  if (mappedInput.hasTouch() && !finishOnBack) {
    activeNav().selected = 0;
  }
}
...
void SettingsActivity::navigateButtons() {
  ...
  buttonNavigator.onNextRelease(
      [this] { moveRingTo(ButtonNavigator::nextIndex(std::max(0, ringPos() - 1), settingsCount) + 1); });
  buttonNavigator.onPreviousRelease(
      [this] { moveRingTo(ButtonNavigator::previousIndex(std::max(0, ringPos() - 1), settingsCount) + 1); });
}
