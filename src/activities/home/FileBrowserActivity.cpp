...
void FileBrowserActivity::rebuildRowItems() {
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    if (mode == Mode::Books && !files[i].empty() && files[i].back() != '/' && !isSupportedFile(files[i])) {
...
void FileBrowserActivity::activateSelected(const bool forceDelete) {
  ...
  if (mode == Mode::Books && (forceDelete || mappedInput.getHeldTime() >= GO_HOME_MS)) {
    ...
    if (isDirectory) {
      ...
      if (mode == Mode::Books && manga::MangaBook::isMangaFolder(dirPath)) {
        lock.unlock();
      ...
      if (mode == Mode::Books && !isSupportedFile(entry)) return;
