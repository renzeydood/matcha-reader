...
static bool recoverySession = false;
...
void silentRestartToReader(const bool forceRestart) {
  if (deepSleepInProgress) return;  // sleeping supersedes the heap-defrag reboot
#if FREEINK_CAP_TOUCH
  if (!forceRestart && finishWifiSessionWithoutRestart()) return;
#endif
...
void setupDisplayAndFonts(bool seamless = false, bool loadSdFonts = true) {
#if !FREEINK_MCU_C3
  ...
  if (loadSdFonts) sdFontSystem.begin(renderer);
...
void setup() {
  ...
  const bool needsBootValidation = HalBootValidation::begin();
  ...
  if (!Storage.begin()) {
    ...
    activityManager.setRecoveryMode();
    ButtonNavigator::setMappedInputManager(mappedInputManager);
    setupDisplayAndFonts(false, false);
    activityManager.goToFullScreenMessage(std::string(tr(STR_ERROR_MSG)) + " " + tr(STR_SD_CARD), EpdFontFamily::BOLD);
    return;
  }
  ...
  if (recoveryFirmwareMode) {
    ...
    ButtonNavigator::setMappedInputManager(mappedInputManager);
    setupDisplayAndFonts(false, false);
    auto updater = makeUniqueNoThrow<SdFirmwareUpdateActivity>(renderer, mappedInputManager, true);
    if (!updater) {
      LOG_ERR("MAIN", "OOM: recovery updater");
      activityManager.goToFullScreenMessage(tr(STR_MEMORY_ERROR));
      return;
    }
    activityManager.replaceActivity(std::move(updater));
    LOG_INF("MAIN", "Recovery started with built-in fonts and default settings");
    return;
  }
  ...
  const BootResume resume = needsBootValidation    ? BootResume::Splash
  ...
  switch (resume) {
    ...
    case BootResume::Splash:
      if (!needsBootValidation) activityManager.goToBoot();
      break;
  }
  ...
  if (needsBootValidation && !rebootedFromPanic) {
    // First boot of an update must not resume a book or translation.
  ...
  if (needsBootValidation && !rebootedFromPanic) {
    ...
    HalBootValidation::confirmHealthyBoot();
  }
...
void loop() {
  ...
  if (recoverySession) {
    ...
    // The held wake gesture must be released before a deliberate recovery exit.
    static bool powerReleased = false;
    const bool powerHeld = mappedInputManager.isPressed(MappedInputManager::Button::Power);
    if (!powerHeld) powerReleased = true;
    if (powerReleased && powerHeld && gpio.getPowerButtonHeldTime() >= 2000) {
      LOG_INF("MAIN", "Leaving recovery by power-button restart");
      ESP.restart();
      return;
    }
